    ...
    public class FEMapCreatorTilesetDiscoveryCoreTests
    {
        ...
        public void DiscoverTilesets_FlagsPngCompatibility()
        {
            ...
            try
            {
                ...
                byte[] badCrc = CreatePngHeader(512, 16);
                badCrc[^1] ^= 0xFF;
                File.WriteAllBytes(Path.Combine(assetsRoot, "bad-crc.png"), badCrc);
                File.WriteAllText(Path.Combine(assetsRoot, "bad-crc.json"), "{}");
                ...
                string json = JsonSerializer.Serialize(new
                {
                    ...
                    tilesets = new object[]
                    {
                        ...
                        {
                            name = "BadCrc",
                            imagePath = "bad-crc.png",
                            generationDataPath = "bad-crc.json",
                            hasImage = true,
                            hasGenerationData = true,
                            diagnostic = "",
                        },
                ...
                FEMapCreatorTilesetInfo badCrcInfo = result.Tilesets.Single(x => x.Name == "BadCrc");
                ...
                Assert.False(badCrcInfo.IsCompatible);
                Assert.Contains("CRC", badCrcInfo.Diagnostic, StringComparison.OrdinalIgnoreCase);
                Assert.Single(result.UsableTilesets);
            }
            finally
            {
        ...
        static byte[] CreatePngHeader(int width, int height)
        {
            ...
            header[27] = 0;
            header[28] = 0;
            WriteUInt32BigEndian(header, 29, ComputeCrc(header, 12, 17));
            return header;
        }
