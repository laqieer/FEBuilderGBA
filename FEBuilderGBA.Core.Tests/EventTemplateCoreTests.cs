...
namespace FEBuilderGBA.Core.Tests
{
    ...
    public class EventTemplateCoreTests
    {
        ...
        // ---- #1591: context-required template substitution (real ROM) ------
        ...
        public void RealRom_FE8U_BrowserWithContext_CondTemplate_SubstitutesAndRoundTrips()
        {
            ...
            try
            {
                ...
                var cond = templates.Find(t => t.Filename.IndexOf("_COND_", StringComparison.Ordinal) >= 0
                                            && t.RequiresContext);
                Assert.NotNull(cond);

                // Empty host = no labels used, "no map" (Cond doesn't need a map).
                ...
                // With host => real substituted codes, no residual placeholder.
                var r = EventTemplateCore.TryGenerateBrowserTemplateWithContext(rom, cond, host, out byte[] bin);
                Assert.Equal(EventTemplateCore.GenerateResult.Ok, r);
                Assert.NotNull(bin);
                Assert.True(bin.Length >= 4);

                var rc = EventTemplateCore.TryGenerateBrowserTemplateCodesWithContext(rom, cond, host, out var codes);
                Assert.Equal(EventTemplateCore.GenerateResult.Ok, rc);
                Assert.NotEmpty(codes);

                // Codes round-trip back to the generated bytes (whole commands).
                var concat = new List<byte>();
                foreach (var c in codes)
                    if (c?.ByteData != null) concat.AddRange(c.ByteData);
                Assert.Equal(bin, concat.ToArray());
        ...
        public void RealRom_FE8U_FindMapID_ResolvesAndDrivesEndEvent()
        {
            ...
            try
            {
                ...
                // back to a real map id (the WinForms FindMapID round-trip).
                var entries = EventScriptReferenceScanner.EnumerateEventEntries(rom);
                if (entries.Count == 0) return; // scanner gated off (no CommentCache wiring etc.)

                var entry = entries[0];
                bool found = EventEditorHostContext.FindMapID(rom, entry.addr, out uint mapid);
                Assert.True(found);
                Assert.Equal(entry.tag, mapid);

                // An address that is NOT an event entry => no map (refuse), not chapter 0.
                bool foundBad = EventEditorHostContext.FindMapID(rom, 0x1, out uint _);
                Assert.False(foundBad);
            }
            finally
            {
                CoreState.ROM = savedRom;
        ...
        // A minimal real host for the context overloads: no labels used + "no map"
        // (so map-required templates refuse, which the _COND_ test does not need).
        sealed class EmptyHost : IEventEditorHostContext
        {
            public bool TryGetMapID(out uint mapid) { mapid = 0; return false; }
