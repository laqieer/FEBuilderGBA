...
namespace FEBuilderGBA.Core.Tests
{
    ...
    public class DecreaseColorSubFlagTests
    {
        ...
        static DecreaseColorCore.QuantizeResult? Quantize(byte[] rgba, int width, int height, int maxColors = 16,
            bool noScale = false, bool noReserve1stColor = false, bool ignoreTSA = false)
        {
            var previous = CoreState.ImageService;
            try
            {
                CoreState.ImageService = new MapConvertImageService();
                return DecreaseColorCore.Quantize(rgba, width, height, maxColors, noScale, noReserve1stColor, ignoreTSA);
            }
            finally
            {
                TestRequire.RestoreImageService(previous);
            }
        }
        ...
        public void NoReserve1stColor_AllPixelsSameIndex()
        {
            ...
            var result = Quantize(rgba, 2, 2, 16, noReserve1stColor: true);
        ...
        public void NoReserve1stColor_ColorCountHasNoTransparentSlot()
        {
            ...
            var resultDefault = Quantize(rgba, 2, 2, 16);
            var resultNoReserve = Quantize(rgba, 2, 2, 16, noReserve1stColor: true);
        ...
        public void DefaultFlags_ReserveFirstColor()
        {
            ...
            var result = Quantize(rgba, 2, 2, 16);
            Assert.NotNull(result);
        ...
        {
            byte[] rgba = MakeSolidRGBA(2, 2, 255, 0, 0);
            var result = Quantize(rgba, 2, 2, 16, noScale: true);
            Assert.NotNull(result);
        ...
        public void NoScale_PaletteStillConvertedToGBAFormat()
        {
            ...
            var result = Quantize(rgba, 1, 1, 16, noScale: true);
            Assert.NotNull(result);
        ...
        public void IgnoreTSA_ProducesResult()
        {
            ...
            var result = Quantize(rgba, 2, 2, 16, ignoreTSA: true);
            Assert.NotNull(result);
        ...
        public void AllFlags_Combined_ProducesResult()
        {
            ...
            var result = Quantize(rgba, 4, 4, 16,
        ...
        public void NoReserve1stColor_TransparentPixelsMappedToIndex0()
        {
            ...
            var result = Quantize(rgba, 2, 1, 16, noReserve1stColor: true);
            Assert.NotNull(result);
