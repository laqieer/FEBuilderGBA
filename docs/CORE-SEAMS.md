...
- `GenerateRandomMapDialogViewModel.cs` / `GenerateRandomMapWorkflow.cs` (Avalonia orchestration seam, #1978 Phase 3) — the modal dialog is the ONLY Avalonia consent surface that may launch FEMapCreator, and only from its explicit `Discover Tilesets` and `Generate` buttons; opening the dialog, typing a path, and browse pickers are spawn-free. Dialog-side external work is isolated to `Task.Run` + injected `ProcessRunnerDelegate`; every `async void` command catches provider/process faults, logs the full exception, and surfaces a localized dialog error instead of rethrowing on the Avalonia dispatcher. Before opening, the editor captures the ROM reference, selected map, pointer entry/target, and decompressed bytes; the UI-thread continuation revalidates that identity immediately before writing and aborts if another window loaded a ROM or changed the map. Desktop title-bar close and single-view back navigation are rejected while work is busy, while the operation's own successful close remains allowed. Runtime Core failures are wrapped in translated formatted messages without discarding their technical detail; each input exposes a localized accessibility name, and error text inherits the active theme foreground instead of hardcoding a low-contrast color. `ApplyGeneratedMapOnUiThreadAsync` marshals ROM/undo/cache work to `Dispatcher.UIThread` under exactly one Begin/Commit transaction and uses strict refresh/palette callbacks whose failures throw into rollback; ordinary UI refreshes retain their log-and-continue behavior. `TryApplyGeneratedMap` keeps `UndoService.Commit()` LAST so any post-write failure rolls back and reloads the cache from reverted ROM bytes.
