...
- `RandomMapGeneratorCore.cs` / `FEMapCreatorLauncherCore.cs` / `FEMapCreatorTilesetDiscoveryCore.cs` / `RandomMapGeneratorMarParserCore.cs` (Core, READ-ONLY external-process seam, #1978 Phase 1; future Avalonia/CLI Random Map Generator) — FEBuilderGBA NEVER vendors FEMapCreator; it only launches a user-selected EXISTING fully-qualified local Windows `.exe`, executable-bit native file (non-Windows, including dotted self-contained names such as `FE_Map_Creator.Cli`), or managed `.dll` (`DOTNET_HOST_PATH` host only when it is executable on Unix, else bare `dotnet`; URLs, drive/root-relative shapes, non-executable native files, PATH lookup for the user path, and shells are rejected). Discovery rejects raw `..` before lexical normalization, then parses `tilesets list --json`, confines every image/data path under the effective root through physical resolution (so symlink/junction escapes fail closed), and requires both confined assets plus a complete valid 512-pixel-wide PNG IHDR/CRC. Generation accepts only FEMapCreator's `experimental` (default), `legacy`, and `hybrid` algorithms and runs `generate --width --height --tileset --algorithm --seed --output <private>.mar --format mar --require-complete --force` (+ optional `--assets-dir`) inside `Path.GetTempPath()/FEBuilderGBA-mapgen-<guid>` with a fixed 120000 ms timeout and 1000000-char per-stream capture cap; `finally` always best-effort deletes that exact temp directory and logs cleanup failure via `Log.Error`. CLI generation first resolves the selected name through the same discovery/compatibility gate and rejects physical/symlink/hard-link collisions between `--out` and the program/image/generation-data files before generation or publication. Non-zero external exits append bounded stderr, stdout, or the runner's containment/capture error (in that priority order) to the surfaced error, so GUI/CLI callers receive the actionable failure reason. The produced headerless signed-little-endian FEMapCreator MAR (`tileIndex*32`) is NEVER copied directly into FEBuilder — one fixed-size buffer is filled from one non-write-shared stream, with EOF and trailing-byte rejection, before `RandomMapGeneratorMarParserCore` requires exact `w*h*2` bytes, each value `>=0`, divisible by 32, and in chipset range, then converts via `raw/32 -> chipsetIndex -> MapEditorTilesetCore.ChipsetIndexToMar(chipsetIndex)` (golden pipeline: raw `0,32,992,1024,32736` → FEBuilder MAR `0,4,124,128,4092`). Public entry points never throw; failures surface as typed categories (`InvalidPath`, `HostUnavailable`, `ProcessStartFailed`, `TimedOut`, `OutputLimitExceeded`, `NonZeroExit`, `OutputMissing`, `ParseFailed`, `IncompatibleTileset`) with bounded stdout/stderr tails for UI/CLI diagnostics.
- `GenerateRandomMapDialogViewModel.cs` / `GenerateRandomMapWorkflow.cs` (Avalonia orchestration seam, #1978 Phase 3) — the modal dialog is the ONLY Avalonia consent surface that may launch FEMapCreator, and only from its explicit `Discover Tilesets` and `Generate` buttons; opening the dialog, typing a path, and browse pickers are spawn-free. Dialog-side external work is isolated to `Task.Run` + injected `ProcessRunnerDelegate`; every `async void` command catches provider/process faults, logs the full exception, and surfaces a localized dialog error instead of rethrowing on the Avalonia dispatcher. Before opening, the editor re-derives the selected map pointer entry from the live map-setting PLIST/table, then captures the ROM reference, map address/ID, entry/target, and decompressed bytes; the UI-thread continuation re-derives and revalidates the same identity immediately before writing, aborting if another window loaded a ROM, changed the selection/PLIST/table, or modified the map bytes. Desktop title-bar close and single-view back navigation are rejected while work is busy, while the operation's own successful close remains allowed. Runtime Core failures are wrapped in translated formatted messages without discarding their technical detail; each input exposes a localized accessibility name, and error text inherits the active theme foreground instead of hardcoding a low-contrast color. `ApplyGeneratedMapOnUiThreadAsync` marshals ROM/undo/cache work to `Dispatcher.UIThread` under exactly one Begin/Commit transaction and uses strict refresh/palette callbacks whose failures throw into rollback; ordinary UI refreshes retain their log-and-continue behavior. `TryApplyGeneratedMap` keeps `UndoService.Commit()` LAST so any post-write failure rolls back and reloads the cache from reverted ROM bytes.
