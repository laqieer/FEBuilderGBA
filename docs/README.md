...
| [MCP-SERVER.md](MCP-SERVER.md) | **`cli-anything-febuildergba` MCP stdio server** — dependency-free JSON-RPC 2.0 adapter over the CLI harness's `Session`/core wrappers: setup, `.mcp.json`, the 21 tools + 3 resources, protocol versions/errors/batching, session semantics, safety annotations, and output bounds. |
