...
├── References: FEBuilderGBA.Core + FEBuilderGBA.SkiaSharp
└── Cross-platform CLI: 72 commands (--version, --help, --build-font-library, --playtest, --makeups,
...
The CLI now exposes **72 commands** (full list via `FEBuilderGBA.CLI --help`).
...
- **Coverage** — The CLI exposes **72 commands** (`--version`, `--build-font-library`, `--playtest`, `--makeups`, `--lint`, `--rebuild`, `--build-buildfile`, `--buildfile-roundtrip`, `--disasm`, `--export-data`, `--export-asset`, and more — full list via `FEBuilderGBA.CLI --help`). Commands no longer require WinForms Form classes; the needed logic lives in `FEBuilderGBA.Core`.
