...
└── Cross-platform CLI: 68 commands (--version, --help, --makeups,
...
The CLI now exposes **68 commands** (full list via `FEBuilderGBA.CLI --help`).
...
- **Coverage** — The CLI exposes **68 commands** (`--version`, `--makeups`, `--lint`, `--rebuild`, `--disasm`, `--export-data`, `--export-asset`, and more — full list via `FEBuilderGBA.CLI --help`). Commands no longer require WinForms Form classes; the needed logic lives in `FEBuilderGBA.Core`.
