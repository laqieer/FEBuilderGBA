...
- `FEBuilderGBA.Android` and `FEBuilderGBA.iOS` are the native mobile **heads** of the Avalonia GUI (opt-in `EnableAndroidTarget` / `EnableIosTarget` on `FEBuilderGBA.Avalonia`, default OFF). Both are **intentionally excluded from `FEBuilderGBA.sln`** (the required `build` check runs on windows-latest with no mobile workload) and built by their own advisory workflows (`android.yml` / `ios.yml`). Each supplies its own entry point (`MainActivity` / `AppDelegate`), single-view lifetime, and first-run `config/` extraction (bundled asset → app-private dir via the pure `FEBuilderGBA.Core\AndroidConfigExtractorCore` + `DirectoryAssetSource`, then `App.BaseDirectoryOverride`). **Preview — build-validated, on-device runtime unvalidated.** See [docs/ANDROID.md](../docs/ANDROID.md) / [docs/IOS.md](../docs/IOS.md).
