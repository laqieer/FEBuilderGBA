...
{
    /// <summary>
    /// Integration (real-ROM) smoke test for the Tiled (.tmx / .tmj) export→import
    /// round-trip (#1387, #1796). Loads FE8U, finds a renderable map, serializes it to
    /// a .tmx / .tmj via <see cref="MapTmxCore.SerializeTmx"/> / <see cref="MapTmxCore.SerializeTmj"/>,
    /// re-parses with <see cref="MapTmxCore.ParseTmx"/> / <see cref="MapTmxCore.ParseTmj"/>,
    /// and asserts the decoded MAR grid is
    ...
    public class MapEditorTmxRomTests
    {
        ...
        [SkippableFact]
        public void Tmj_FE8U_ExportImport_RoundTrips()
        {
            string? romPath = TestRomLocator.FindRom("FE8U");
            Skip.If(romPath == null, "FE8U.gba not available — skipping real-ROM .tmj round-trip test.");
            ...
            RomTestHelper.WithRom("FE8U", () =>
            {
                ...
                // Export -> .tmj (JSON), then re-parse and assert the MAR grid round-trips.
                string tmj = MapTmxCore.SerializeTmj(snapshot, "fe8u.tsx");
                Assert.False(string.IsNullOrEmpty(tmj));

                Assert.True(MapTmxCore.ParseTmj(tmj, out int w, out int h, out ushort[] back, out string err), err);
                Assert.Equal(vm.MapWidth, w);
                Assert.Equal(vm.MapHeight, h);
                Assert.Equal(expected, back);
