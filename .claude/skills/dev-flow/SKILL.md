...
12. **Check for ALL comments** — both inline threads AND issue-level conversation comments:
    ...
    # Issue-level conversation comments (human, Copilot bot, AND Copilot CLI — DO NOT IGNORE)
    gh api repos/laqieer/FEBuilderGBA/issues/<N>/comments --jq '.[] | select(.user.login != "github-actions[bot]") | "\(.user.login): \(.body | split("\n")[0])"'
    ```
    **CRITICAL: Always check BOTH.** Human reviewers, Copilot bot, and Copilot CLI ALL post non-inline comments here. Ignoring them is a recurring failure mode.
