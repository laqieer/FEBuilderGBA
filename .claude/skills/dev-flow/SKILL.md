...
3. **Trigger the Review Gate** on the plan (pick your branch — see `DEVELOPMENT-WORKFLOW.md` → **Developer & Reviewer Roles**):
   ...
   ```
   - **Branch B** (Copilot CLI → its own model board): convene the in-session board (`claude-opus-4.8`, `gpt-5.5`, `gemini-3.5-flash`) per `DEVELOPMENT-WORKFLOW.md` → Developer & Reviewer Roles → Branch B; synthesize; post the consolidated review as an issue comment via `gh`. **Never** `agency cc`.
...
11. **Trigger the Review Gate** on the PR (pick your branch — see `DEVELOPMENT-WORKFLOW.md` → **Developer & Reviewer Roles**):
    - **Branch A** (Claude Code CLI → Copilot CLI):
    ```bash
    ...
    ```
    - **Branch B** (Copilot CLI → its own model board): convene the in-session board (`claude-opus-4.8`, `gpt-5.5`, `gemini-3.5-flash`) per `DEVELOPMENT-WORKFLOW.md` → Developer & Reviewer Roles → Branch B, applying the **same** rubric as the Branch-A prompt above (screenshots, GUI Test Report, test-plan all-`[x]`, scope); synthesize; post a `## Cross-Model Review Board` PR comment via `gh`. **Never** `agency cc`.
...
14. **Repeat 11-13** until no unaddressed feedback across all three channels.
