...
3. **Trigger the Review Gate** on the plan (pick your branch — see `DEVELOPMENT-WORKFLOW.md` → **Developer & Reviewer Roles**):
   ...
   GITHUB_COPILOT_PROMPT_MODE_REPO_HOOKS=true copilot -p "Review the plan comment on issue #<N> in laqieer/FEBuilderGBA. Post your review findings as a comment on the issue. After you finish posting the review, prune any git worktree you created for this review: run 'git worktree prune' and 'git worktree remove --force' any checkout you made under your session-state directory. Include your Copilot CLI version and model at the end." --autopilot --enable-all-github-mcp-tools --allow-all-tools
   ```
   > Set the literal `"true"` (not `1`/unset) before this external launch so the repo's `preToolUse` context-budget hook (issue #1995) is active; verify with `/env` in interactive sessions.
