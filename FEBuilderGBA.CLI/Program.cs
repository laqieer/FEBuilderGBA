...
namespace FEBuilderGBA.CLI
{
    static class Program
    {
        ...
        static void PrintHelp()
        {
            ...
            Console.WriteLine("    --kind=objtiles        OBJ tileset LZ77 block — exports the DECOMPRESSED 4bpp payload (needs --addr=<DEREFERENCED OBJ LZ77 stream hex>, NOT RomInfo.map_obj_pointer); FE7 obj2 is a separate stream/--addr; NOT chipset TSA/config, NOT tile animations 1/2 (#1360)");
            ...
            Console.WriteLine("    --kind=map|mapchange|objtiles  map = .mar layout (+ .mar.json); mapchange = raw u16 overlay block (.change + .change.json, identity copy) (#1355); objtiles = decompressed 4bpp OBJ payload (.objtiles + .objtiles.json, identity copy) (#1360)");
            ...
            Console.WriteLine("    --kind=map|mapchange|objtiles  map = lossless u16 .mar layout; mapchange = structure-exact .change overlay (#1355); objtiles = structure-exact .objtiles decompressed body (#1360)");
            Console.WriteLine("    --in=<x.mar|x.change|x.objtiles>  Input asset file");
            Console.WriteLine("  --verify-asset           ROM-backed mismatch proof for a map-change OVERLAY or OBJ tileset (reads ROM READ-ONLY; never mutates) (#1355/#1360)");
