...
{
    /// <summary>
    /// ViewModel backing <see cref="Views.SkillConfigFE8NVer2SkillView"/>.
    /// Phase 1/2/4/5/6 gap-sweep parity raise (#396).
    ///
    /// FE8N v2 is a third skill-patch flavour (distinct from SkillSystems and
    /// CSkillSys 0.9.x). It stores skill metadata in a single contiguous table
    /// at a base resolved via byte-pattern scan:
    /// <list type="bullet">
    ///   <item><description><c>rom.u32(0x89268+4)</c> must be a safe GBA pointer (iconExPointer
    ///   sentinel — when missing, the patch isn't installed).</description></item>
    ///   <item><description>Grep for <c>[0x50,0x93,0x08,0x08, 0x48,0x93,0x08,0x08]</c> in the
    ///   <c>0xE00000..0</c> scan window. The hit lands `4 * 5` bytes after the
    ///   start of the iconPointers array, so <c>iconPointers = hit - 20</c>.</description></item>
    ///   <item><description>The 5th valid pointer (skipping out-of-range &lt; 0xE00000 slots)
    ///   is <c>g_SkillBaseAddress</c> — the address of the slot, NOT the
    ///   dereferenced offset. We store both forms below.</description></item>
    ///   <item><description><c>ICON_LIST_SIZE</c> (row stride) is detected at runtime from
    ///   <c>iconPointers + 4 * 11</c> (valid range 16..40, multiple of 4).
    ///   Default sizeof-16 layout omits the Item2 (P16) field; sizeof-20+
    ///   layout includes it.</description></item>
    /// </list>
    /// Row layout (sizeof-16, default):
    /// <code>
    /// u16 textId @ +0
    /// u16 palette @ +2
    ...
    /// </code>
    /// Sizeof-20+ adds <c>u32 item2-skill-pointer (P16) @ +16</c>.
    ///
    /// Animation pointers live in a separate table at <see cref="AnimeBaseAddress"/>
    /// (resolved during the same scan via two paths depending on which sizeof
    /// the ROM ships — see <c>SkillConfigFE8NVer2SkillForm.FindSkillFE8NVer2IconPointersLow</c>).
    ///
    ...
    /// </summary>
    public partial class SkillConfigFE8NVer2SkillViewModel : ViewModelBase, IDataVerifiable
    {
        // Default row stride; overridden at LoadList time.
        ...
        uint _item2SkillPointer; // Only meaningful when IconListSize >= 20.
        ...
        /// <summary>
        /// Detected row stride (a.k.a. <c>ICON_LIST_SIZE</c>). Range 16..40,
        /// multiple of 4. Defaults to 16 before the first LoadList. The
        /// View binds the Item2 row's <c>IsVisible</c> to <see cref="HasItem2"/>.
        /// </summary>
        public uint IconListSize { get => _iconListSize; set => SetField(ref _iconListSize, value); }

        /// <summary>True iff the detected stride supports the Item2 (P16) field.</summary>
        public bool HasItem2 => _iconListSize >= 20;

        public uint BlockSize => _iconListSize;

        /// <summary>
        /// Address of the iconPointers slot that holds the skill-table GBA
        /// pointer (WF <c>g_SkillBaseAddress</c>). Populated by
        ...
        /// <summary>
        /// Dereferenced skill-table base offset (0 if the patch isn't installed).
        /// </summary>
        public uint SkillBaseAddress { get => _skillBaseAddress; set => SetField(ref _skillBaseAddress, value); }

        /// <summary>
        /// Dereferenced anime-pointer-table base offset (0 if the patch isn't
        ...
        /// 0x08000000 high bit, <c>write_p32</c> re-adds it). 0 = no animation.
        /// Read from <c>animeBase + 4 * id</c>.
        /// </summary>
        ...
        public List<AddrResult> LoadList()
        {
            ...
            // Force a fresh scan on every LoadList (cheap and avoids stale
            // pointers after a Patch install/uninstall).
            PreviewIconHelper.ResetFE8NVer2Cache();

            uint loc = PreviewIconHelper.FindSkillFE8NVer2SkillPointerLocation();
            uint baseAddr = PreviewIconHelper.FindSkillFE8NVer2SkillBaseAddress();
            uint animeBase = PreviewIconHelper.FindSkillFE8NVer2AnimeBaseAddress();
            uint stride = PreviewIconHelper.GetFE8NVer2IconListSize();

            SkillPointerLocation = loc;
            SkillBaseAddress = baseAddr;
            AnimeBaseAddress = animeBase;
            // Out-of-range strides shouldn't be possible (the helper validates
            // them), but defensively fail-closed if so.
            if (stride < 16 || stride > 40 || stride % 4 != 0)
            {
                ResetDerivedListState();
                return new List<AddrResult>();
            }
            ...
            for (uint i = 0; ; i++)
            {
                ...
                // Cap the iteration at a sane maximum even if a corrupt ROM
                // never returns 0xFF.
                if (i >= 256) break;
        ...
        /// Mirrors WinForms <c>SkillConfigFE8NVer2SkillForm.GetSkillText(id)</c>:
        ...
        public void LoadEntry(uint addr)
        {
            ROM rom = CoreState.ROM;
            if (rom == null) return;
            // Use the detected stride (defaults to 16 if we haven't scanned yet).
            ...
            {
                SkillPointerLocation = PreviewIconHelper.FindSkillFE8NVer2SkillPointerLocation();
                SkillBaseAddress = PreviewIconHelper.FindSkillFE8NVer2SkillBaseAddress();
                AnimeBaseAddress = PreviewIconHelper.FindSkillFE8NVer2AnimeBaseAddress();
                uint detectedStride = PreviewIconHelper.GetFE8NVer2IconListSize();
                if (detectedStride >= 16 && detectedStride <= 40 && detectedStride % 4 == 0)
                {
            ...
            Item2SkillPointer = stride >= 20 ? ReadPointerOffset(rom, addr + 16) : 0;
        ...
        /// (f) u32 item2-skill-pointer @ +16 (only when stride &gt;= 20)
        /// (g) u32 animation pointer @ animeBase + 4*id (via write_p32)
        ///
        ...
        public void Write()
        {
            ...
            // (f) item2-skill-pointer (only when stride supports it).
            if (stride >= 20)
            {
        ...
        public Dictionary<string, string> GetDataReport()
        {
            return new Dictionary<string, string>
            {
                ...
                ["Item2SkillPointer"] = HasItem2 ? $"0x{Item2SkillPointer:X08}" : "-",
        ...
        public Dictionary<string, string> GetRawRomReport()
        {
            ...
            if (HasItem2)
            {
                report["u32@0x10"] = $"0x{rom.u32(a + 16):X08}";
            }
            return report;
        }
        ...
        public Dictionary<string, string> GetFieldOffsetMap()
        {
            ...
            if (HasItem2)
            {
                map["Item2SkillPointer"] = "u32@0x10";
            }
            return map;
        }
    }
