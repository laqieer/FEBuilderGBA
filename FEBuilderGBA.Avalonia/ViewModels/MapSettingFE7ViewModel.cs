...
{
    public class MapSettingFE7ViewModel : ViewModelBase, IDataVerifiable
    {
        uint _currentAddr;
        string _name = "";
        bool _isLoaded;
        ...
        uint _fortuneTextOpening, _fortuneTextEliwood;
        uint _fortuneTextHector, _fortuneTextConfirm;
        uint _fortunePortrait, _fortuneFee;
        uint _prepScreenChNo1, _prepScreenChNo2;
        uint _chapterNumber;
        ...
        public uint FortuneTextOpening { get => _fortuneTextOpening; set => SetField(ref _fortuneTextOpening, value); }
        public uint FortuneTextEliwood { get => _fortuneTextEliwood; set => SetField(ref _fortuneTextEliwood, value); }
        public uint FortuneTextHector { get => _fortuneTextHector; set => SetField(ref _fortuneTextHector, value); }
        public uint FortuneTextConfirm { get => _fortuneTextConfirm; set => SetField(ref _fortuneTextConfirm, value); }
        // W118-W124 are word-sized fortune teller text IDs in FE7
        ...
        public void LoadMapSetting(uint addr)
        {
            ...
            if (dataSize > 119) FortuneTextOpening = rom.u16(addr + 118);
            if (dataSize > 121) FortuneTextEliwood = rom.u16(addr + 120);
            if (dataSize > 123) FortuneTextHector = rom.u16(addr + 122);
            if (dataSize > 125) FortuneTextConfirm = rom.u16(addr + 124);
            if (dataSize > 126) FortunePortrait = rom.u8(addr + 126);
            if (dataSize > 127) FortuneFee = rom.u8(addr + 127);
        ...
        public void WriteMapSetting()
        {
            ...
            if (undo != null)
            {
                undoData = undo.NewUndoData("MapSettingFE7");
            ...
            if (dataSize > 119) rom.write_u16(addr + 118, FortuneTextOpening);
            if (dataSize > 121) rom.write_u16(addr + 120, FortuneTextEliwood);
            if (dataSize > 123) rom.write_u16(addr + 122, FortuneTextHector);
            if (dataSize > 125) rom.write_u16(addr + 124, FortuneTextConfirm);
            if (dataSize > 126) rom.write_u8(addr + 126, FortunePortrait);
            if (dataSize > 127) rom.write_u8(addr + 127, FortuneFee);
        ...
        public Dictionary<string, string> GetDataReport()
        {
            var r = new Dictionary<string, string>
            {
                ...
                ["FortuneTextOpening"] = $"0x{FortuneTextOpening:X04}",
                ["FortuneTextEliwood"] = $"0x{FortuneTextEliwood:X04}",
                ["FortuneTextHector"] = $"0x{FortuneTextHector:X04}",
                ["FortuneTextConfirm"] = $"0x{FortuneTextConfirm:X04}",
