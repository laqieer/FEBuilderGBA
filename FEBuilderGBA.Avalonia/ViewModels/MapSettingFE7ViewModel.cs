...
namespace FEBuilderGBA.Avalonia.ViewModels
{
    public class MapSettingFE7ViewModel : ViewModelBase, IDataVerifiable
    {
        ...
        uint _b116, _b117;
        // --- W118, W120, W122, W124 ---
        uint _w118, _w120, _w122, _w124;
        // --- B126-B135 ---
        uint _b126, _b127, _b128, _b129, _b130, _b131, _b132, _b133, _b134, _b135;
        // --- W136, W138 ---
        uint _w136, _w138;
        // --- B140-B147 ---
        ...
        // B116, B117: event ID PLIST, world map auto event
        ...
        public uint W120 { get => _w120; set => SetField(ref _w120, value); }
        ...
        public void LoadEntry(uint addr)
        {
            ...
            if (dataSize > 121) W120 = rom.u16(addr + 120);
        ...
        public void WriteMapSetting()
        {
            ...
            if (dataSize > 121) rom.write_u16(addr + 120, W120);
        ...
        public Dictionary<string, string> GetRawRomReport()
        {
            ...
            // B116, B117
            if (ds > 116) report["u8@116"] = $"0x{rom.u8(a + 116):X02}";
            if (ds > 117) report["u8@117"] = $"0x{rom.u8(a + 117):X02}";
            // W118, W120, W122, W124
            if (ds > 119) report["u16@118"] = $"0x{rom.u16(a + 118):X04}";
            if (ds > 121) report["u16@120"] = $"0x{rom.u16(a + 120):X04}";
            if (ds > 123) report["u16@122"] = $"0x{rom.u16(a + 122):X04}";
            if (ds > 125) report["u16@124"] = $"0x{rom.u16(a + 124):X04}";
            // B126-B135
            for (uint i = 126; i < 136 && i < ds; i++)
                report[$"u8@{i}"] = $"0x{rom.u8(a + i):X02}";
