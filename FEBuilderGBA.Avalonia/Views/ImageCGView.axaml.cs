...
{
    public partial class ImageCGView : TranslatedUserControl, IEmbeddableEditor, IDataVerifiableView
    {
        ...
        void LoadList()
        {
            try
            {
                ...
                EntryList.SetItemsWithIcons(items, i => ListIconLoaders.CGThumbnailLoader(items, i));
            }
            catch (Exception ex)
            {
        ...
        async void ImportPng_Click(object? sender, RoutedEventArgs e)
        {
            try
            {
                ...
                uint addr = _vm.CurrentAddr;
                // P0=image, P4=TSA, P8=palette (matching WinForms ImageCGForm)
                var importResult = ImageImportCore.Import3Pointer(rom, loadResult.IndexedPixels, loadResult.GBAPalette,
                    loadResult.Width, loadResult.Height, addr + 0, addr + 4, addr + 8);
        ...
        {
            await ImageDisplay.ExportPng(TopLevel.GetTopLevel(this) as Window, "cg_image.png");
        }
