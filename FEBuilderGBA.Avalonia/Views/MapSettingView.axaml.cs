...
{
    public partial class MapSettingView : TranslatedUserControl, IEmbeddableEditor, IDataVerifiableView
    {
        ...
        public EditorDescriptor Descriptor => new("Map Settings", 1773, 1038, SizeToContent: global::Avalonia.Controls.SizeToContent.WidthAndHeight);
        ...
        // cancel / malformed / zero / same count this is a no-op. FE8 uses its
        // own MapList control + LoadMapSettingList() count path.
        async void OnExpandListClick(object? sender, RoutedEventArgs e)
        {
            try
            {
                ...
                uint current = (uint)_vm.LoadMapSettingList().Count;
                if (current == 0)
                {
                ...
                {
                    _undoService.Rollback();
                    Log.Error("MapSettingView.OnExpandListClick inner failed: " + inner.ToString());
                    CoreState.Services?.ShowError(R._("Map setting list expansion failed: {0}", inner.Message));
                }
            }
            catch (Exception ex)
            {
                Log.Error("MapSettingView.OnExpandListClick failed: " + ex.ToString());
            }
        }
