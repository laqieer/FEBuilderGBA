...
- `EventScriptEditorCore.cs` (Core, #1435; WF `EventScriptInnerControl`) — editable `List<OneCode>` engine: `BuildFromRom`/`ScanLength` (es-driven, not `SearchEveneLength`), `Insert`(TERM-before)/`Delete`/`Move{Up,Down}`/`InsertRange`/`NewCodeFromScript`(`CloneScriptDefaultByte`)/`ImportFromText`/`Serialize`(auto-term)/`WriteAll` (`isSafetyOffset`+ALIGN4 gate; in-place else `FindFreeSpace`+`RepointAllReferences`+`NotifyChangePointer`, one ambient undo, **refs==0 REFUSAL**, byte-identical restore). Avalonia `EventScriptViewModel`/`EventScriptView` toolbar; `IsWorldMapEvent`/`IsTopLevelEvent` carried through nav (WorldMapEventPointer/EventCond jumps `SetEventKind`); `WriteAll` via `CommitExternal` (no nested scope). Script-type agnostic; Procs/AI view-wiring deferred.
