...
- `PatchMetadataCore` clean-ROM-diff uninstall (Core, ROM-MUTATING, #1462; WF `PatchForm.UninstallPatchInner`) — `CollectPatchRegions`(out untraceableCount)/`RomContainsPatch`/`IsCompatibleRom`/`UninstallPatchWithCleanRom`: uninstall a prior-session BIN patch (no per-patch backup) by diffing vs a user-picked patch-free ROM. GBA-header (0xA0..0xB0) compat gate + patch-absence check BEFORE mutation; CORRECTION-ONLY restore writes only DIFFERING bytes so over-estimated region/JUMP lengths never clobber neighbours; EA/`$FREEAREA`/`$GREP` ⇒ PARTIAL report (never over-claim); ambient undo, byte-identical rollback. Wires the orphaned Avalonia `PatchFormUninstallDialogView` into `PatchManagerView` Uninstall (backup fast-path kept; `CanUninstall` no longer backup-gated).
