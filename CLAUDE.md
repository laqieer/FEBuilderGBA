...
- `NameResolver.GetFaceTranslateInfo` + `ToolTranslateROMCore.AppendAIHintMessage` (Core, READ-ONLY) — Text Editor AI Hints export (#1028): `GetTranslateInfoByFaceID` (unit-table-by-face-at-+6, `(f2&0x80)==0x20`) + `AppendAIHintMessage` (escape forms, dedup, mob fallback); Avalonia `ExportAllTexts`
...
- `DecompProject.cs` (Core, READ-ONLY, never-throws) — decomp open mode (#1129): `Detect`+`ResolveBuiltRom`; `IsDecompMode`; CLI `--project`. #1130 `DecompSymbolResolver` (`--resolve-addr`); #1131 `DecompDiffMigrationCore` (`--migrate-diff`); #1133 `DecompAssetExportCore`+`IndexedPngWriter` (`--export-asset`). #1132/#1141/#1148 `DecompSourceWriterCore` C+JSON writers (items/units(=characters)/classes/map_settings; signed,multifield); chapter scalars source-backed, pointers+raw map ASSETs ROM-only/manual (`DecompMapAssetGuard`). #1150 `DecompRoundTripAuditCore` (`--decomp-audit` coverage matrix)+`NmmSchemaBridgeCore` (`--nmm-to-manifest`/`--manifest-to-nmm`; pointer/odd-size flagged, never dropped)+`IndexedPngReader`/`DecompAssetValidatorCore` (`--validate-asset`, no ROM)
