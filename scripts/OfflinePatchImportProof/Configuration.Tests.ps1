...
function Invoke-RejectedRootClassSerializationTests {
    ...
    for($i=0;$i -lt $samples.Length;$i++){
        ...
        foreach($shape in @('compact','pretty','nested')){
            ...
            Assert-Proof ($decoded.Count -eq 33 -and $decoded.ContainsKey('RejectedRootClass') -and
...
function New-ProofRestageFixture([string]$Root,[ValidateSet('None','PurePassed','CompilePassed','BindingAccepted','OuterSource','HistoryExtra','HistoryMissing')][string]$InvalidType='None',[object[]]$CompileReferences) {
    ...
    $validationRows=@(foreach($name in @('Pure.json','Compile.json','Policy.compile-only.dll','Desktop.compile-only.dll','a.dat','b.dat','c.dat','d.dat','e.dat','f.dat','g.dat')){
        $value=switch($name){
            'Pure.json' {@{passed=$true;cases=528;bindingCases=22;nativeCalls=$false;appLaunched=$false}}
    ...
    foreach($stage in @('Validate','Build','Inputs')){
        ...
        if($stage -ceq 'Validate'){$outer.bindingCases=22;$outer.pureCases=528;$outer.runtimeBindingsAccepted=7;$outer.outputOnlyDlls=2}
