...
function Run([string]$exe,[string[]]$arguments,[int]$seconds,[string]$cwd,[bool]$maySpawn=$false) {
    ...
    $script:sequence++
    $prefix=Join-Path $control ('process-'+$script:sequence)
...
try {
    ...
    if ($Stage -ceq 'Build') {
        ...
        foreach ($mode in @('Pure','Compile')) {
            $null=Run "$PSHOME\pwsh.exe" @('-NoLogo','-NoProfile','-NonInteractive','-STA','-File',"$Code\validate-helper.ps1",'-Configuration',$Configuration,'-ConfigurationSha256',$ConfigurationSha256,'-Mode',$mode,'-Id',$Id,'-SourceManifestSha256',$SourceManifestSha256,'-HelperSourceManifestSha256',$HelperSourceManifestSha256) 90 $control
