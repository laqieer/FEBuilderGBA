...
function ProofEnvelope {
    ...
    function Invoke-ProofSupervision {
        ...
        try {
            ...
            $childMode=switch($Mode){Pure {'Pure'};Restage {'RestageChild'};default {'PrerequisitesChild'}}
            $childValues=@{}
            if($Mode -cne 'Pure'){
                $childValues=@{configuration=$Configuration;configurationSha256=$ConfigurationSha256;priorId=$configurationData.restage.priorId}
                if($Mode -ceq 'Restage'){$childValues.newGuiId=$NewGuiId}
            }
            $childArguments=New-PinnedProofChildArguments -ChildMode $childMode -Path (Join-Path $outer 'child-bindings.json') -Values $childValues
