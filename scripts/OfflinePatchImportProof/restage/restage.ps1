...
function ProofEnvelope {
    function Invoke-RestageEntry {
        ...
        function PriorEvidence {
            ...
            Assert-RInteger $v.processImageCases 10 10;Assert-RInteger $pure.cases 534 534;Assert-RInteger $pure.bindingCases 22 22
            Assert-R ($pure.passed -is [bool] -and $pure.passed -and $pure.cases -eq 534 -and $pure.bindingCases -eq 22 -and
            ...
            foreach($key in @('bindingCases','pureCases','runtimeBindingsAccepted','outputOnlyDlls')) {
                $expectedCount=@{bindingCases=22;pureCases=534;runtimeBindingsAccepted=7;outputOnlyDlls=2}[$key]
