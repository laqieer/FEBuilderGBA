...
# cdef stream, token-normalizes safe parser-only GCC qualifiers, removes only
# allowlisted ABI-neutral GNU attributes, and fails closed on any drift,
# ambiguity, mismatch, containment violation, unsupported/layout attributes,
# excessive aliases/inline blocks/token/attribute replacements, or cleanup
# failure.
