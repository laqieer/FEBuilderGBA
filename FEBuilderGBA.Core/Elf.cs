...
*/
        ...
        string CheckLostLabelSYM(byte[] bin, uint baseaddr, uint e_shoff)
        {
            ...
            for (uint n = 0; n < nloop_count; n++)
            {
                ...
                sb.AppendLine(string.Format("行方不明のラベル({0})が存在します。", name2));
            }
            return sb.ToString();
        }
        ...
        {
            string testPath = Path.Combine(CoreState.BaseDirectory ?? ".", "config", "data", "test.elf");
            Elf elf = new Elf(testPath, useHookMode: false);
