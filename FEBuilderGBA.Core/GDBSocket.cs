...
//            return Encoding.ASCII.GetString(resBytes, 0, resSize);
        ...
        internal string UnPacket(byte[] packet, int length)
        {
            if (length <= 5)
            {
        ...
        internal byte[] MakePacket(string order)
        {
            ...
            sendBuffer[i + 1] = (byte)'#';
            sendBuffer[i + 2] = NibbleToHexChar((byte)((sum >> 4) & 0xf));
            sendBuffer[i + 3] = NibbleToHexChar((byte)((sum) & 0xf));
            return sendBuffer;
        }


        static byte NibbleToHexChar(byte nibble)
        {
