    ...
    public class MagicSplitUtil
    {
        ...
        static magic_split_enum SearchMagicSplitLow()
        {
            PatchDetection.PatchTableSt[] table = new PatchDetection.PatchTableSt[] {
                new PatchDetection.PatchTableSt{ name="FE8NMAGIC",	ver = "FE8J", addr = 0x2a542,data = new byte[]{0x30 ,0x1C}},
                new PatchDetection.PatchTableSt{ name="FE7UMAGIC",	ver = "FE7U", addr = 0x68DE0,data = new byte[]{0x38 ,0x18 ,0x01 ,0x78}},
                new PatchDetection.PatchTableSt{ name="FE8UMAGIC",	ver = "FE8U", addr = 0x2BB44,data = new byte[]{0x01 ,0x4B ,0xA5 ,0xF0 ,0xC1 ,0xFE}},
                new PatchDetection.PatchTableSt{ name="FE8UMAGIC",	ver = "FE8U", addr = 0x17E5C,data = new byte[]{0x01 ,0x4B ,0xB9 ,0xF0 ,0x35 ,0xFD}}, //魔力分離パッチ FE8U 2022年以降
            };
            ...
            foreach(PatchDetection.PatchTableSt t in table)
            {
                if (t.ver != version)
                {
                    continue;
        ...
        static void FE7UInit()
        {
            string filename ;
            uint p;
            filename = Path.Combine(CoreState.BaseDirectory,"config", "patch2", "FE7U", "FE7-Str Mag Split", "Autolevelling and Saves", "Char Mag Autolevel.dmp");
            p = U.Grep4EndByDmp(filename, 0x100, 0);
            if (U.isSafetyOffset(p))
            {
                UnitTag = CoreState.ROM.p32(p);
            }

            filename = Path.Combine(CoreState.BaseDirectory, "config", "patch2", "FE7U", "FE7-Str Mag Split", "Autolevelling and Saves", "Class Mag Autolevel.dmp");
            p = U.Grep4EndByDmp(filename, 0x100, 0);
            if (U.isSafetyOffset(p))
            {
                ClassTag = CoreState.ROM.p32(p);
            }
        }
        static void FE8UInit()
        {
            uint p;
            {
                ...
                p = U.GrepEnd(CoreState.ROM.Data, bin, CoreState.ROM.RomInfo.compress_image_borderline_address, 0, 4, 0, true);
                if (U.isSafetyOffset(p))
                {
                    UnitTag = CoreState.ROM.p32(p);
                    ClassTag = CoreState.ROM.p32(p + 4);
                    return;
                }
            }
        ...
        static void WriteUnitAs(uint uid, uint skip,uint newValue,Undo.UndoData undodata)
        {
            ...
            CoreState.ROM.write_u8(a, newValue , undodata);
        }
        ...
        static public uint GetUnitBaseMagicExtends(uint uid, uint addr)
        {
            ...
            {
                return CoreState.ROM.u8(addr + 50);
            }
            return 0;
        }
        static public uint GetUnitGrowMagicExtends(uint uid, uint addr)
        {
            ...
            {
                return CoreState.ROM.u8(addr + 51);
            }
            return 0;
        }
        ...
        static public uint GetClassBaseMagicExtends(uint cid, uint addr)
        {
            ...
            {
                return CoreState.ROM.u8(addr + 80);
            }
            return 0;
        }
        ...
        static public uint GetClassGrowMagicExtends(uint cid, uint addr)
        {
            ...
            {
                return CoreState.ROM.u8(addr + 81);
            }
            return 0;
        }
        static public uint GetClassLimitMagicExtends(uint cid, uint addr)
        {
            ...
            {//体力を使いまわす
                return CoreState.ROM.u8(addr + 25);
            }
            return 0;
        }
        static public uint GetClassPromotionGainMagicExtends(uint cid, uint addr)
        {
            ...
            {
                return CoreState.ROM.u8(addr + 82);
            }
            return 0;
        }
        static public void MakeEADefine(StringBuilder sb)
        {
                ...
                || g_Cache_magic_split_enum == magic_split_enum.FE8UMAGIC)
            {
                ...
                ExportFunction.One(sb, "MagCharTable", UnitTag);
                ExportFunction.One(sb, "MagClassTable", ClassTag);
            }
        }
