    ...
    public class EventScript
    {
        ...
        void Load(string fullfilename , EventScriptType eventScriptType)
        {
            ...
            if (eventScriptType == EventScriptType.Event)
            {
                ...
                CoreState.EventScriptPatchLoader?.Invoke(scripts, flags, CoreState.TextEscape, CoreState.ExportFunction);
                CoreState.FlagCache?.MargeFlags(flags);
            }
//            else if (eventScriptType == EventScriptType.AI)
//            {
//                //パッチに登録されているイベントのスキャン(少し重いです)
...
//                                 Debug.Assert(false);
        ...
        public static uint SearchEveneLength(byte[] romdata,uint start_addr,bool isWorldMapEvent = false)
        {
            ...
            while (true)
            {
                EventScript.OneCode code = CoreState.EventScript.DisAseemble(romdata, addr);
        ...
        public static int GetAffiliation(EventScript.OneCode code, int current)
        {
            ...
            for (int n = 0; n < code.Script.Args.Length; n++)
            {
                ...
                {
                    EventScript.GetArg(code, n, out v);
                    if (CoreState.ROM.RomInfo.version == 7)
                    {
        ...
        public static bool IsExitCode(EventScript.OneCode code, uint addr, uint lastBranchAddr)
        {
            ...
            if (lastBranchAddr == 0)
            {//ダミーではない本当の終端...?
                if (CoreState.ROM.RomInfo.version == 8 && IsFE8DummyEnd(code,addr) )
                {//ダミー終端
                    return false;
                }
                return true;
        ...
        static bool IsFE8DummyEnd(EventScript.OneCode code, uint addr)
        {
            ...
            if (nextAddr + 0x4 >= CoreState.ROM.Data.Length)
            {//ROM終端
                return false;
            }
            uint a1 = CoreState.ROM.u8(nextAddr + 0);
            uint a2 = CoreState.ROM.u8(nextAddr + 1);
            uint a3 = CoreState.ROM.u8(nextAddr + 2);
            uint a4 = CoreState.ROM.u8(nextAddr + 3);
...
//            }
            ...
            uint a5 = (CoreState.ROM.u8(nextAddr + 4) & 0xF0);
            if (! (a5 == 0x20 || a5 == 0x40))
            {//ダミー終端ではない
                return false;
            }
