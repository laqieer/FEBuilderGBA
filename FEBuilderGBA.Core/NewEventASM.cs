    ...
    public class NewEventASM
    {
        ...
        public OneCode DisAseemble(byte[] data,uint startaddr)
        {
            ...
            for(int i = 0 ; i < Scripts.Length ; i++)
            {
                ...
                {//マッチ
                     code.ByteData    = U.getBinaryData(data, startaddr, (uint)Scripts[i].Size);
        ...
        public static string GetArg(OneCode code, int arg_count, out uint v)
        {
            ...
            if (arg.Size == 1)
            {//1バイト
                v = U.u8(code.ByteData, (uint)arg.Position);
            }
            else if (arg.Size == 2)
            {//2バイト
                v = U.u16(code.ByteData, (uint)arg.Position);
            }
            else //if (arg.Size == 4)
            {//4バイト
                v = U.u32(code.ByteData, (uint)arg.Position);
            }
