...
namespace FEBuilderGBA
{
    ...
    public class Mod
    {
        ...
        {
            //MOD
            if (CoreState.ROM == null || CoreState.ROM.RomInfo == null)
            {
                return;
            }
            ScanMods(System.IO.Path.Combine(CoreState.BaseDirectory ?? ".", "config", "patch2", CoreState.ROM.RomInfo.VersionToFilename));
        }
        ...
        {
            Mods = new List<ModSt>();
            if (CoreState.ROM == null || CoreState.ROM.RomInfo == null || CoreState.ROM.RomInfo.version == 0)
            {
                return;
            }

            // OptionForm.lang() is WinForms-only; use CoreState.Language
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    {//MODでは $GREP4マクロとかは無効. IF_NOTもない.
                     //よって、事前定数になるので、条件にマッチしないMODは無視できる.
                        ...
                        {
                            data[i] = CoreState.ROM.u8(address + (uint)i);
                        }
