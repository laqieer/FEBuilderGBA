...
namespace FEBuilderGBA
{
    public class GrowSimulator
    {
        ...
        {
            int max_level = (int)(CoreState.GetLevelMaxCaps != null ? CoreState.GetLevelMaxCaps() : 20u);
            if (CoreState.IsHighClass != null && CoreState.IsHighClass(classs_id))
        ...
        {
            int cc_count = CoreState.GetCCCount != null ? CoreState.GetCCCount(shien_classs_id) : 0;

            int level;

            //最大値LV
            int max_level = (int)CoreState.ROM.u8(CoreState.ROM.RomInfo.max_level_address);
            if (cc_count >= 2)
            {//見習いからのCC
