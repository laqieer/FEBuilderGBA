...
namespace FEBuilderGBA
{
    ...
    public class DisassemblerTrumb
    {
        ...
        public DisassemblerTrumb(IAsmMapFile asmMapFile)
        {
        ...
        string CommentFunction(uint addr)
        {
            ...
            {
                return "   //" + p.Name + U.SA(p.ResultAndArgs)+ CoreState.CommentCache.S_At(addr);
            }
            string comment;
            if (CoreState.CommentCache.TryGetValue(addr, out comment))
            {
                return "  //" + comment;
            }
        ...
        {
            return "   //BIOS: " + GbaBiosCall.GetSWI_GBA_BIOS_CALL(swi);
        }
        ...
        {
            byte[] bin = File.ReadAllBytes(Path.Combine(CoreState.BaseDirectory ?? ".", "testdata", "MMBPrepStats.dmp"));
        ...
        {
            return IsCode(CoreState.ROM.Data, pos, CoreState.ROM);
        }
        public static bool IsCode(byte[] prog, uint pos)
        {
            return IsCode(prog ,  pos, CoreState.ROM);
        }
        public static void TEST_IsCode_FE8J()
        {
            if (CoreState.ROM.RomInfo.version == 8 && CoreState.ROM.RomInfo.is_multibyte )
            {
