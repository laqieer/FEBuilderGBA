...
namespace FEBuilderGBA
{
    public class TextEscape
    {
        ...
        // MargeExstraEscapeList lives in WinForms layer (needs TextScriptFormCategorySelectForm)
        // Access AddEscapeMapping via this accessor for WinForms to implement it.
        public IReadOnlyDictionary<string, (string feditorAdv, string info)> GetAddEscapeMappingSnapshot()
        {
            var result = new Dictionary<string, (string, string)>();
            foreach (var t in AddEscapeMapping)
            {
                result[t.Key] = (t.Value.feditorAdv, t.Value.info);
            }
            return result;
        }
