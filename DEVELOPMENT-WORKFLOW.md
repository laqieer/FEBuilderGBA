...
- **Conventional-commit subjects, ≤ 100 chars.** Use a valid type (`build|chore|ci|docs|feat|fix|perf|refactor|revert|style|test`) and keep the header ≤ 100 characters — CI's `Lint commit messages` (`commitlint.config.mjs`) enforces this, and fixing an over-length subject after pushing needs an interactive rebase + `git push --force-with-lease` (or a squash-merge), so it's simpler to **catch it before committing** by installing the opt-in local hook *in this worktree* (pre-commit provisions its own Node on first install; needs network): `pip install pre-commit && pre-commit install --hook-type commit-msg` (commit-msg only — do NOT add `--hook-type pre-commit` here: the ggshield stage blocks commits unless ggshield is authenticated (`ggshield auth login` or `GITGUARDIAN_API_KEY`), which an ephemeral worktree usually isn't). See [CONTRIBUTING.md](CONTRIBUTING.md#commit--pr-title-convention).
