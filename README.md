...
| `FEBuilderGBA.CLI` | net9.0 | Cross-platform CLI tool (40 commands: `--version`, `--help`, `--makeups`, `--applyups`, `--lint`, `--lint-oam`, `--disasm`, `--disasm-event`, `--decreasecolor`, `--pointercalc`, `--rebuild`, `--songexchange`, `--convertmap1picture`, `--translate`, `--translate-roundtrip`, `--translate_batch`, `--export-data`, `--import-data`, `--data-roundtrip`, `--render-portrait`, `--export-portrait-all`, `--import-portrait`, `--generate-font`, `--export-midi`, `--import-midi`, `--apply-patch`, `--list-patches`, `--list-resources`, `--uninstall-patch`, `--expand-table`, `--merge3`, `--compile-event`, `--freespace`, `--hex-dump`, `--search-text`, `--resolve-names`, `--lastrom`, `--force-detail`, `--test`, `--testonly`; flags: `--force-version`, `--noScale`, `--noReserve1stColor`, `--ignoreTSA`, `--table`, `--patch-name`) |
...
├── FEBuilderGBA.CLI/            net9.0    (cross-platform CLI — 40 commands)
