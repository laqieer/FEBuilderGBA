...
**Avalonia desktop save shortcut:** on the desktop `MainWindow` only, exact `Ctrl+S` (Windows/Linux) or `⌘S` (macOS) invokes the existing **Save ROM** action when that menu item is enabled. Separate editor windows keep their own shortcuts, and the single-view Android/iOS/browser shells continue to use their own Save UI.
