...
| `FEBuilderGBA.CLI` | net9.0 | Cross-platform CLI tool (13 commands: `--version`, `--help`, `--makeups`, `--applyups`, `--lint`, `--disasm`, `--decreasecolor`, `--pointercalc`, `--rebuild`, `--songexchange`, `--convertmap1picture`, `--translate`; flags: `--force-version`, `--noScale`, `--noReserve1stColor`, `--ignoreTSA`) |
| `FEBuilderGBA.SkiaSharp` | net9.0 | SkiaSharp implementation of IImageService (GBA 4bpp/8bpp tiles, palette conversion) |
| `FEBuilderGBA.Avalonia` | net9.0 | Cross-platform Avalonia UI (ROM loading, 59 editors: unit/item/class/CC branch/terrain name with read/write; map settings/text/song table/portrait/move cost/support unit/support attribute/support talk/event cond/map change/exit point/map pointer/tile animation viewers; 8 item detail viewers; 3 arena/3 monster/2 summon/3 sound/2 menu/2 credits/3 world map viewers; 10 image/graphics viewers; image viewer; lint runner; categorized navigation) |
...
- 🧪 **Test Coverage:**
  ...
  - CLI arg parsing tests (all 13 commands with complete argument sets)
