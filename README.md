...
- 🧰 **Cross-platform CI reliability:** a dependency-free contract runs before the .NET jobs, every build/test/publish command disables persistent MSBuild/Roslyn servers, and CLI smoke checks invoke the already-built Release DLL. This mitigates hosted-runner process/handle hangs without weakening compile or test failures.
