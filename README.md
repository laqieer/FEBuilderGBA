...
| `FEBuilderGBA.Core.Tests` | net9.0 | Cross-platform Core unit tests (runs on Linux/macOS/Windows). Includes the SkiaSharp Android version-guard (declared/runtime/restored-graph 2.88.x pin) + render byte-parity smoke tests (image decode/PNG exact, font within tolerance) that the advisory emulator-parity CI (#1125) now runs on API-34 x86_64 + x86 emulators |
