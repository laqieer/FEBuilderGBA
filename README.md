...
# Decomp OBJ tileset LZ77 decompressed-payload source (#1360): exports the DECOMPRESSED 4bpp payload,
...
# FE7 obj2 split is out of scope. NOT chipset TSA/config, NOT tile animations 1/2.
...
| Map Editor | map_asset_binaries | Raw map asset save (GUI: OBJ/TSA/anim/map-change) | ManualMigration | GUI raw-ROM-save path for the remaining LZ77/pointer map binaries (chipset TSA/config, tile animations 1/2) AND the 12-byte map-change RECORD chain (terminator/flagID/PLIST metadata) — NOT the map-change overlay tile data block (which is source-backed export/import/verify above) and NOT the OBJ tileset (source-backed export/import/verify above) and NOT the .mar tile layout; migrate these via --export-asset |
