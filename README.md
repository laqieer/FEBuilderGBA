...
| `FEBuilderGBA.CLI` | net9.0 | Cross-platform CLI tool (33 commands: `--version`, `--help`, `--makeups`, `--applyups`, `--lint`, `--lint-oam`, `--disasm`, `--disasm-event`, `--decreasecolor`, `--pointercalc`, `--rebuild`, `--songexchange`, `--convertmap1picture`, `--translate`, `--translate-roundtrip`, `--translate_batch`, `--export-data`, `--import-data`, `--data-roundtrip`, `--render-portrait`, `--export-portrait-all`, `--export-midi`, `--apply-patch`, `--list-patches`, `--list-resources`, `--uninstall-patch`, `--expand-table`, `--merge3`, `--resolve-names`, `--lastrom`, `--force-detail`, `--test`, `--testonly`; flags: `--force-version`, `--noScale`, `--noReserve1stColor`, `--ignoreTSA`, `--table`) |
